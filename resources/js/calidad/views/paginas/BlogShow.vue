<template>
   <b-container :fluid="width >= 762 ? false : true">
      <b-row class="mt-1">
         <!-- blogs -->
         <b-col cols="12">
            <b-card :img-src="`/storage/blog-banner/${contenido.banner}`" img-top :img-alt="contenido.titulo"
               :title="contenido.titulo">
               <b-media no-body v-if="contenido.usuario">
                  <b-media-aside vertical-align="center" class="mr-50">
                     <b-avatar href="javascript:void(0)" size="24" v-if="contenido.usuario"
                        :src="contenido.usuario.avatar" />
                  </b-media-aside>
                  <b-media-body>
                     <small class="text-muted mr-50">Por</small>
                     <small>
                        <b-link class="text-body">{{ `${contenido.usuario.nombre} ${contenido.usuario.apellido}`}}
                        </b-link>
                     </small>
               ...
               <div class="d-flex align-items-center justify-content-between">
                  <div class="d-flex align-items-center">
                     <div class="d-flex align-items-center mr-1">
                        <b-link class="mr-50" href="#blogComment">
                     ...
                     <div class="d-flex align-items-center" v-if="usuario.id">
                        <b-link class="mr-50"
                           @click="alternarFavorito({contenido_id:contenido.id,usuario_id:usuario.id},isFavorito(usuario.id))">
                  ...
                  <div class="blog-detail-share">
                     <b-dropdown variant="link" toggle-class="p-0" no-caret right>
                        ...
                        <b-dropdown-item v-for="social in socialShareIcons" :key="social.icon"
                           @click="compartir(social)">
                           <feather-icon :icon="social.icon" size="18" v-if="social.enableIcon" />
                           {{ social.label }}
                        </b-dropdown-item>
                     </b-dropdown>
                  </div>
         ...
         <b-col id="blogComment" cols="12" class="mt-1">
            ...
            <b-card>
               <b-container fluid>
                  <b-row>
                     ...
                     <b-col cols="12" md="8">
                        <b-form-input v-model="searchQuery" type="search" placeholder="Buscar comentario..."
                           size="sm" />
                     </b-col>
            ...
            <template v-if="!loading">
               <VuePerfectScrollbar class="scroll-area" :settings="{maxScrollbarLength: 60}">
                  <b-card v-for="(comentario,index) in comentarios.filter(val => val.comentario_id == null)"
                     :key="index">
                     <b-media no-body>
                        ...
                        <b-media-body>
                           ...
                           <b-media class="ml-2" no-body v-for="(comment,idx) in comentario.comentarios" :key="idx">
                              <b-media-aside class="mr-75">
                     ...
                     <b-row>
                        ...
                        <b-col sm="6">
                           <b-form-group class="mb-2">
                              ...
                              <validation-provider name="telefono" #default="{valid,errors}">
                                 <the-mask v-model="comentario.telefono" :mask="'+############'" class="form-control"
                                    type="tel" masked placeholder="+57 314 1245689" />
                        ...
                        <b-col cols="12">
                           <b-form-group class="mb-2">
                              ...
                              <validation-provider name="comentario" rules="required" #default="{valid,errors}">
                                 <b-form-textarea v-model="comentario.comentario" placeholder="Comentario..."
                        ...
                        <b-col cols="12">
                           <b-form-checkbox id="checkbox-1" v-model="commentCheckmark" name="checkbox-1" class="mb-2">
                              Guarde mi nombre, correo electrónico y telefono en este navegador para la próxima vez que
                              comente.
                           </b-form-checkbox>
                        </b-col>
                        <b-col cols="12">
   ...
   metaInfo() {
      return {
         ...
         meta: [{
            vmid: 'description',
            name: 'description',
            content: this.contenido.contenido.substring(0, 157) + '.. '
         }]
      }
   },
   ...
   setup(props, { emit }) {
      ...
      const cargarForm = () => {
         ...
         if (!contenidos.value.length) {
            store.dispatch('contenido/fetchPublic', slug.value)
         } else {
            store.commit('contenido/capturarSlug', slug.value)
         }
         ...
         if(contenido.value.id){
            comentario.value.contenido_id = contenido.value.id
         }
      ...
      const compartir = ({ label }) => {
         ...
         switch (label) {
            case "WhatsApp":
               var url = `https://wa.me/?text=Mirén este contenido Increible que quiero compartir con ustedes. ${window.location.href}`
      ...
      const enviarComentario = () => {
         ...
         store.dispatch('comentario/guardar', comentario.value).then(({ result, comentario: comment }) => {
            ...
            if (result) {
               ...
               comentario.value.contenido_id = contenido.value.value
               refetchData()
      ...
      return {
         ...
         alternarFavorito,
         usuario,
         width: computed(() => store.state.app.windowWidth),
         isFavorito: computed({
